...
2. Fetch the target ticket. Its `Repo` property routes you: backend → api/, frontend → mobile-app/, website → website/ (see workspace CLAUDE.md table for that repo's DEFAULT_BRANCH, TEST_CMD, LINT_CMD). ALL git/test/gh commands run inside that folder (`cd` first). Never run git at workspace root.
