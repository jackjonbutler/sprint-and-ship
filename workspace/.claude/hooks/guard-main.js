#!/usr/bin/env node
// Workspace-level guard: blocks git commit/push on a protected branch of any your product repo.
const { execSync } = require("child_process");
const path = require("path");
const PROTECTED = {
  "api": ["development", "main"],
  "mobile-app": ["development", "main", "flutterflow"],
  "website": ["master"],
};
let input = "";
  ...
  try {
    ...
    for (const r of targets) {
      ...
      if (PROTECTED[r].includes(branch)) {
        console.error(`BLOCKED: ${r} is on protected branch '${branch}'. Cut a ticket branch first (feat/tkt-<id>-<slug>). Workspace CLAUDE.md rule 1.`);
        process.exit(2);
      }
    }
    process.exit(0);
