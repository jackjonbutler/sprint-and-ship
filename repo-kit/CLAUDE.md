# CLAUDE.md — your product Dev Pipeline

This repo is part of the your product one-man dev team system. Tickets live in Notion, code lives here, PRs go through GitHub.

## Repo config (edit per repo)

- REPO_NAME: website  <!-- must match the "Repo" select in Notion the tasks database: frontend | backend | website -->
- DEFAULT_BRANCH: main
- TEST_CMD: npm test
...
- Tickets: the tasks database — data source `collection://{{TASKS_DB_COLLECTION_ID}}`
- Sprints: the sprints database — data source `collection://{{SPRINTS_DB_COLLECTION_ID}}`
- Ticket key = the `ID` property (auto-increment). Refer to tickets as TKT-<ID>.
...
1. NEVER commit directly to DEFAULT_BRANCH. All work happens on `feat/tkt-<id>-<slug>`, `fix/tkt-<id>-<slug>`, or `chore/tkt-<id>-<slug>` branches.
2. One ticket = one branch = one PR. PR title: `TKT-<id>: <ticket title>`. PR body links the Notion ticket URL and includes a test plan.
...
8. Keep commits small and conventional: `feat(scope): ...`, `fix(scope): ...`, referencing TKT-<id> in the body.

## Commands

- /triage — find Needs Plan tickets for this repo, research, write plans, mark Plan Ready
- /plan-ticket TKT-<id> — research + plan a single ticket
- /sprint — execute the Current sprint's approved tickets for this repo
- /work TKT-<id> — implement one approved ticket end to end (branch → code → tests → PR)
- /ship TKT-<id> — merge the PR, verify deploy, close the ticket
