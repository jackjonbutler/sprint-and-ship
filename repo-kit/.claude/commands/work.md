---
description: Implement one approved ticket end to end (usage - /work TKT-123)
...
1. Fetch ticket $ARGUMENTS from the tasks database (`collection://{{TASKS_DB_COLLECTION_ID}}`).
...
## Setup
4. Create branch `feat/tkt-<id>-<kebab-slug>` (or fix/ / chore/ based on ticket Tags).
...
7. Work in small commits — one plan step or logical unit per commit, conventional format, TKT-<id> in the body.
...
9. If the plan turns out to be wrong or incomplete: make the smallest sensible adjustment, then update the plan section in Notion with a "### Deviations" note. If the deviation is large (different approach, new dependency, schema change), stop, set `AI Stage` = "Blocked", comment on the ticket, and wait for the user.
...
14. Report: what was built, what to look at in the preview, anything the user should test by hand.
