...
## Pick up the pointer
1. Fetch the "Current" sprint page from the sprints database (`collection://{{SPRINTS_DB_COLLECTION_ID}}`). Read its "## Execution Order (REPO_NAME)" manifest.
...
4. Fetch the ticket. Hard gates: `AI Stage` = "Plan Approved", `Repo` = REPO_NAME, clean `git status`, DEFAULT_BRANCH up to date. Gate fails → mark the manifest line "⚠ skipped: <reason>", move it under Completed with that note, and stop (the user decides).
5. Branch `feat/tkt-<id>-<slug>` off DEFAULT_BRANCH. Notion: `AI Stage` = "Building", `Status` = "In progress", `Branch` set.
...
8. Push; `gh pr create` (title `TKT-<id>: <title>`, body links ticket + test plan). Notion: `AI Stage` = "PR Open", `Status` = "QA", comment with PR URL.

## Transition (GSD-style checkpoint)
9. Update the sprint manifest: check the ticket off and move its line to "### Completed" with a one-line summary: "TKT-<id> — <what shipped> — PR <url>".
10. STOP. End with the completion protocol, exactly:
    "✅ TKT-<id> done — PR: <url>
    Sprint: <n> of <total> complete. Next up: TKT-<id2> — <title>.
    → /clear, then /next"
