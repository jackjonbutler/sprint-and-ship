...
Jack Butler / Trips Together · July 2026
...
Your existing `Tasks - Tech` database is the single source of truth. Two orthogonal properties track a ticket:
...
Sprints stay exactly as you run them in `Sprints - Tech`: you assemble a sprint by relating approved tickets to the Current sprint. Claude never assigns tickets to sprints.
...
**6 · Your QA + ship.** The PR arrives titled TT-<id> with the ticket linked, a test plan, and (for web) a Vercel preview URL in the ticket comments. You click around, review the diff at whatever depth the ticket deserves, then /ship TT-123: checks green → squash-merge → deploy verified against a smoke URL (Vercel prod) or build triggered (EAS for mobile) → failed deploy auto-reverts and Blocks the ticket → success closes it with merge SHA and changelog comment.
