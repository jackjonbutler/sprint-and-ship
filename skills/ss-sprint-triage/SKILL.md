...
Plannable = AI Stage empty, "Needs Plan", or "Blocked"-with-answered-questions. Already-approved tickets are context, not targets — never rewrite an approved plan without flagging it. Plan in the body's wave order and report progress per wave. There is no ticket cap — plan a whole wave, however big. It is fine for one run to plan the first waves and leave later ones for the next run, but say clearly which tickets are planned and which remain; never leave a wave half-planned without saying so.
