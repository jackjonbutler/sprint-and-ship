...
while true; do
  ...
  echo "$RESP" | jq -c '.result[]?' | while read -r upd; do
    ...
    case "$CMD" in
      plan|"run plan"|triage)  run_locked "$CHAT" "Sprint planning" bin/plan-sprint.sh & ;;
      wave|"plan wave")        run_locked "$CHAT" "Sprint planning (one wave)" bin/run-job.sh sprint-triage & ;;
      "plan "*|"run plan "*)
        ...
        SPRINT_TARGET="$TARGET" run_locked "$CHAT" "Sprint planning ($TARGET)" bin/plan-sprint.sh & ;;
      ...
      help)
        reply "$CHAT" "Commands: <b>plan</b> (whole sprint, wave by wave) · <b>plan V2.2</b> (a named sprint) · <b>wave</b> (one wave only) · <b>build</b> · <b>merge</b> (land green PRs into development) · <b>status</b> · or answer questions as TKT-123: your answer" ;;
      tt-*|TT-*|tkt-*|TKT-*)
